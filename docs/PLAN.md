...
| 11 | QY | Query completion (342), part 1: per-shape point/ray/distance/contact/closest-points queries | QY1 running (brief `qy1-shape-queries.md`: parry's `distance` / `closest_points` / `contact` for the closed set, missing `PointQuery` / `RayCast` members, `Aabb` queries; golden family `shape_queries`; nothing on the step's path). After BT4: free parity items and the remaining step levers (impact tick: narrow phase 101k, island solve 194k) |
