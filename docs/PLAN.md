...
| 7 | CP1 | Convex polygon shape (≤ 8 vertices): `Shape::ConvexPolygon` (approved interface change), AABB, mass, analytic point projection and ray cast (upstream: GJK), golden cases | brief `cp1-convex-polygon-shape.md` |
| 7 | CP2 | Polygon contact generators: SAT + clipping vs polygon/cuboid/segment/capsule, ball and half-space through CP1, dispatch, golden manifolds (upstream: PFM–PFM with GJK/EPA) | after CP1 |
