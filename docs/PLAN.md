...
| 11 | LO1 | Parity leftovers of the rigid-body, collider and sensor packages (≈ 64 items, mostly internals already triaged by CW / QY1) | brief `lo1-parity-leftovers.md`, claude sonnet |
| 11 | SH1 | Additional 2D shapes, part 1: triangle, round shapes | brief `sh1-triangle-round-shapes.md` (new `Shape` variants boxed so `Shape` stays six felts; existing shapes' exact steps unchanged; golden families from parry2d-f64); after PO1 (it touches `shape.cairo`), claude opus |
