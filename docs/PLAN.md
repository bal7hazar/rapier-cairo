...
| 12 | CC | CCD and shape casts (91) — **first** (programme, 2026-09-26: CCD on the pebble guards thin planks if slingfall drops to 2 or 1 substeps) | CC1 running (brief `cc1-shape-casts.md`: parry's linear / nonlinear shape casts and sweeps for the closed set incl. SH1's shapes, `World` / `QueryPipeline::cast_shape*`; golden vs parry2d-f64; claude opus); then CC2: rapier's CCD solver in the step for `ccd_enabled` bodies only (zero steps otherwise) |
