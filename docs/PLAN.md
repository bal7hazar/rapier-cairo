...
| 10 | WS | Versioned world state save / restore (`WorldState`, `ArenaState` on the three sets), requested by the programme (spike G1b), blocking the game's G3/G4 | brief `ws-world-state.md`; first after RB merges (needs RB's `RigidBody` layout) |
