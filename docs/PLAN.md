...
| 10 | WS | Versioned world state save / restore (`WorldState`, `ArenaState` on the three sets), requested by the programme (spike G1b), blocking the game's G3/G4 | brief `ws-world-state.md`; running (claude opus) |
| 10 | CW | Collider API completion (59) + Pipeline and world facade (67) | brief `cw-collider-world-api.md`; after WS (shares `world.cairo` and the sets) |
| 10 | G0 | Level-shaped golden scene (8–12 sleeping blocks + cores + 15–25 m/s pebble, 300 ticks, 20-block variant; substeps 4/2/1, 30 Hz) + cost of a level | brief `g0-level-golden-scene.md`; after RB/SE |
| 10½ | BT | Gas per awake body-tick (solver sweeps, sleeping cost, `find_pairs`, substep count) — ahead of waves 11–14 if the owner confirms. Also RB's follow-ups: its no-contact fast path (`pipeline/free_path.cairo`) makes the P3 free-fall probes 6.5–20 % cheaper but hides a +13 % net cost of the general path on contactless worlds (`free_fall32` without it: 16.81M vs 14.88M on main) — find it, then keep or drop the fast path; world construction +3.7 % (≈ +59k gas per inserted body) | sized by G0 |
