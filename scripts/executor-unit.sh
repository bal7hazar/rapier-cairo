...
# output only; the agent log stays in ~/orchestrator/logs/rapier-cairo/<id>.log)   Stop: systemctl --user stop …
...
exec systemd-run --user --collect --quiet --unit "$UNIT" --working-directory "$ROOT" \
  ...
  --setenv=EXECUTOR_LOG_DIR="${EXECUTOR_LOG_DIR:-$HOME/orchestrator/logs/rapier-cairo}" \
