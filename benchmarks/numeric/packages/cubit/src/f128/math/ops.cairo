...
fn div(a: Fixed, b: Fixed) -> Fixed {
    let a_u256: u256 = WideMul::wide_mul(a.mag, ONE_u128);
...
fn mul(a: Fixed, b: Fixed) -> Fixed {
    let res_u256: u256 = WideMul::wide_mul(a.mag, b.mag);
...
fn sqrt(a: Fixed) -> Fixed {
    assert(a.sign == false, 'must be positive');
    let root: u64 = Sqrt::sqrt(a.mag);
    let scale_root: u64 = Sqrt::sqrt(ONE_u128);
