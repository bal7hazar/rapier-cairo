...
| `triangle_contacts.json` | 40 | `<pair>/<regime>` | **SH1** contact manifolds of a triangle, plus `intersection_test` and `distance`, see [SH1 families](#sh1-families) |
| `round_shape_contacts.json` | 56 | `<pair>/<regime>` | **SH1** the same for the round cuboid, triangle and convex polygon |
| `sh1_queries.json` | 30 + 20 + 5 + 10 | `<shape>/<what>` | **SH1** point projections (triangle locations), world-space ray casts, mass properties, AABBs |
...
Generated by `src/sh1.rs`, emitted by `src/cairo/sh1.rs` into files of their own
(`generated/{triangle_contacts,round_shape_contacts,sh1_points,sh1_rays,sh1_mass,sh1_aabb}` and
their `partN` modules, at most 12 cases each), so that no existing table grows. The shapes are
carried by `Sh1ShapeRaw` (`Other(ShapeRaw)`, `Polygon`, `Triangle`, `RoundCuboid`,
`RoundTriangle`, `RoundPolygon`).

- Contacts: 10 triangle pairs and 14 round-shape pairs (both orders for the ball, cuboid and
  half-space ones) × `separated` (gap 0.5), `within_pred` (0.01), `touching` (0, tagged
  ambiguous) and `overlapping` (−0.05, shape 2 turned by 10°). Shape 2 sits below shape 1 (its
  apex under shape 1's centre for a triangle), or above the plane of a half-space first. Every case
  also records `query::intersection_test` and `query::distance` for the same placement.
- Queries: the counter-clockwise triangle `(-1,-0.5) (1,-0.5) (0.25,0.75)`, the same clockwise,
  a round cuboid, a round triangle and a round pentagon (radius 0.1); six points, four rays (the
  hollow `inside` ray uses a unit `dir`, see the `capsule/inside` upstream bug above), density 2,
  two poses.
- Bands (Cairo tests `rapier_geometry2d/tests/sh1_*_golden.cairo`): manifolds, intersection and
  distance within 64 raw (both families); triangle projections, locations and rays within 4 raw;
  round-shape distances and rays within 4096 raw and their projected points within `2^19` raw
  (upstream's GJK stops up to `7e-5` from the exact boundary point, its distance agreeing to
  `1e-8`); the round polygon's dispatch AABB under rotation is checked for containment (see
  `RoundConvexPolygonShape`), its tight method against upstream.
