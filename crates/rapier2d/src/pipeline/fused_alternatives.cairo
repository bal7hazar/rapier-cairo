...
use super::{
    BodyInfo, advance_to_final_positions, body_info, detect_collisions, handle_user_changes,
    no_body_info, snapshot_collider, solve,
};
            ...
            Some(parent) => {
                ...
                if let Some(info) = infos.get(parent.index) {
                    ...
                    if info.handle == parent {
                        dense = Some((info.body_type, info.world_com, info.dominance));
                    }
                }
                match dense {
                    Some(found) => found,
...
pub fn collision_inputs_field_reads(
    ...
    for (handle, collider) in snapshot {
        ...
        let (body_type, world_com, dominance) = parent_info;
...
pub fn advance_with_snapshot_outlined(
    ...
    for (handle, body) in bodies.iter().span() {
        if *body.enabled && *body.body_type != RigidBodyType::Fixed {
            advance_body_outlined(*handle, *body, ref bodies, ref colliders, snapshot);
        }
    }
}
