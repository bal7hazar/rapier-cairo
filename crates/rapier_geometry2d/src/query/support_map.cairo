...
use crate::shape::round_shape::RoundConvexPolygonShapeTrait;
use crate::shape::triangle::triangle_core;
use crate::shape::{
    BallTrait, CapsuleTrait, ConvexPolygon, ConvexPolygonTrait, Cuboid, CuboidTrait, Segment,
    SegmentTrait, Shape, Triangle, TriangleTrait,
};
...
pub fn local_support_point_toward(shape: Shape, dir: Vec2) -> Vec2 {
    match shape {
        ...
        Shape::Triangle(s) => s.unbox().local_support_point(dir),
        Shape::RoundCuboid(s) => crate::shape::support_map::SupportMap::local_support_point_toward(
            s, dir,
        ),
        Shape::RoundTriangle(s) => crate::shape::support_map::SupportMap::local_support_point_toward(
            s.unbox(), dir,
        ),
...
/// The core of a cuboid dilated by `radius`.
fn cuboid_core(s: Cuboid, radius: Fixed) -> Core {
...
/// The core of a triangle (its counter-clockwise core polygon) dilated by `radius`.
fn triangle_core_of(t: Triangle, radius: Fixed) -> Core {
    let (polygon, _) = triangle_core(t);
    let core = polygon_core(polygon);
    Core { radius, ..core }
}

/// The core of the SH1 shapes (triangle and round shapes), out of line.
#[inline(never)]
fn sh1_core(shape: Shape) -> Core {
    match shape {
        Shape::Triangle(s) => triangle_core_of(s.unbox(), ZERO),
        Shape::RoundCuboid(s) => cuboid_core(s.inner_shape, s.border_radius),
        Shape::RoundTriangle(s) => {
            let s = s.unbox();
            triangle_core_of(s.inner_shape, s.border_radius)
        },
        Shape::RoundConvexPolygon(s) => {
            let s = s.unbox();
            Core { radius: s.border_radius, ..polygon_core(s.inner_shape) }
        },
        _ => core::panic_with_felt252(super::errors::NOT_SUPPORT_MAP),
    }
}
